...
> **Windows 주의 — 보안 차단 해제 (필수)**
>
> 인터넷에서 다운로드한 ZIP의 파일은 Windows가 실행을 차단합니다. **창이 아예 안 뜨면 이 작업을 먼저 하세요.**
>
> 1. ZIP 파일 우클릭 → **속성(Properties)**
> 2. 하단의 **"차단 해제(Unblock)"** 체크박스 체크 → **확인**
> 3. 그 다음 ZIP 압축 해제 → `run.bat` 실행
>
> *(이미 압축 해제했다면 각 `.bat` 파일마다 같은 방법으로 차단 해제)*
